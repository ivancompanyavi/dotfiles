...
#
# Each run stamps a new @version and points @updateURL back at the file, so if
# live reload ever misses a change, Stylus picks it up on its own update check
# (or on "Check for update" in its editor).
set -euo pipefail
