...
install_fzf () {
  if !(command_exists fzf); then
    ...
    doo $(brew --prefix)/opt/fzf/install
  else
    installed 'tmux'
  fi
}
...
install_neovim() {
  if !(command_exists nvim); then
    doo brew install neovim
    doo ln -s ~/.config/nvim ~/.config/nvim
...
install_alacritty() {
  if !(command_exists alacritty); then
    ...
    doo ln -s "$EXPORT_DIR/.config/alacritty/alacritty.yml" ~/.config/alacritty/alacritty.yml
